...
class VG(VariantGenerator):
    ...
    @variant
    def meta_batch_size(self): #env.step을 진행하는 env 갯수
        #return [20] # at least a total batch size of 400. (meta batch size*fast batch size)
        return [5] # at least a total batch size of 400. (meta batch size*fast batch size)
...
unityBuildFilePath = "D:/grew_svn/2020/GitBlit/grewRL/marathon-envs-0.5.0a + ml-agents-0.5-3.0a + TensorFlowSharp/UnitySDK/Build/test_maml_1130/Unity Environment.exe"
#for v in variants:
v = variants[0]
#env = TfEnv(normalize(AntEnvRandLeg()))
env = TfEnv(normalize(UnityGymEnv(unityBuildFilePath)))
task_var = 'randleg'
...
run_experiment_lite(
    algo.train(),
    exp_prefix='unity_trpo_maml' + task_var + '_' + str(max_path_length),
