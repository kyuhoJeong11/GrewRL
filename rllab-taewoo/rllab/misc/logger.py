...
def save_itr_params(itr, params):
    if _snapshot_dir:
        if _snapshot_mode == 'all' or _snapshot_mode == '\'all\'':
            file_name = osp.join(_snapshot_dir, 'itr_%d.pkl' % itr)
            joblib.dump(params, file_name, compress=3)
        elif _snapshot_mode == 'last' or _snapshot_mode == '\'last\'':
            # override previous params
            file_name = osp.join(_snapshot_dir, 'params.pkl')
            joblib.dump(params, file_name, compress=3)
        elif _snapshot_mode == 'gap' or _snapshot_mode == '\'gap\'':
            if itr % _snapshot_gap == 0:
                file_name = osp.join(_snapshot_dir, 'itr_%d.pkl' % itr)
                joblib.dump(params, file_name, compress=3)
        elif _snapshot_mode == 'none' or _snapshot_mode == '\'none\'':
            pass
        else:
