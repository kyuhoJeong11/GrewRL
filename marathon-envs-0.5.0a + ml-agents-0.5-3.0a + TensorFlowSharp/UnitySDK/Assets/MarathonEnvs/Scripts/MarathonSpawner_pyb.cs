...
//        private List<GameObject> deactivate = new List<GameObject>();
//        private void DestoryALLChild()
//        {
//            foreach (Transform child in transform)
//            {
//                //Destroy(child.gameObject);
//                deactivate.Add(child.gameObject);
//                child.gameObject.SetActive(false);
//            }
//        }

//        public void ReloadXML(int legCount)
//        {
//            DestoryALLChild();

//            int sensor = legCount;
//            int actuator = legCount * 2;
//            //brain.brainParameters.vectorObservationSize = 9 + sensor + actuator * 5;
//            //brain.brainParameters.vectorActionSize[0] = actuator;

//            StartCoroutine(ReSpawn(0.01f, 5));
//        }

//        IEnumerator ReSpawn(float delayTime, int legCount)
//        {
//            string str = string.Format("legs(3~8)/test_{0}leg", legCount);
...
//            GetComponent<OpenAIAntAgent>().AgentReset();
//            yield break;
//        }
...
//            //if (Force2D)
//            //{
//            //    foreach (var item in GetComponentsInChildren<BRigidBody>())
...
//                    // float r = 0f;
//                    var childRb = item.Joint.GetComponent<BRigidBody>();
//                    if (childRb != null)
//                    {
//                        B6DOFConstraint configurableJoint = item.Joint as B6DOFConstraint;
//                        var t = Vector3.zero;
...
//        List<KeyValuePair<string, BTypedConstraint>> ParseBody(XElement xdoc, GameObject parentBody, GeomItem geom = null,
//            GeomItem parentGeom = null, List<JointDocQueueItem> jointDocsQueue = null)
//        {
//            var joints = new List<KeyValuePair<string, BTypedConstraint>>();
...
//                {
//                    var fixedJoint = parentGeom.Geom.AddComponent<BFixedConstraint>();
//                    fixedJoint.otherRigidBody = geom.Geom.GetComponent<BRigidBody>();
//                }
...
//            var rb = geom.Geom.AddComponent<BRigidBody>();
//            //rb.useGravity = true;
//            //rb.SetDensity(DefaultDensity);
//            rb.mass = rb.mass; // ref: https://forum.unity.com/threads/BRigidBody-setdensity-doesnt-work.322911/
...
//                        // DebugPrint($"{name} {attribute.Name.LocalName}={attribute.Value}");
//                        geom.GetComponent<BRigidBody>().mass = float.Parse(attribute.Value);
//                        break;
//                    case "density": //  "1000"
...
//                        var rb = geom.GetComponent<BRigidBody>();
//                        //rb.SetDensity(density);
//                        rb.mass = rb
//                            .mass; // ref: https://forum.unity.com/threads/BRigidBody-setdensity-doesnt-work.322911/
//                        break;
//                    case "solmix": // "1"
...
//        List<KeyValuePair<string, BTypedConstraint>> ParseJoint(XElement xdoc, GeomItem parentGeom, GeomItem childGeom,
//            GameObject body)
//        {
...
//            var childRidgedBody = childGeom.Geom.GetComponent<BRigidBody>();
//            var parentRidgedBody = parentGeom.Geom.GetComponent<BRigidBody>();
...
//                    jointType = typeof(BHingedConstraint);
//                    break;
//                case "free":
//                    DebugPrint($"ParseJoint: Creating type:{type} ");
//                    jointType = typeof(BFixedConstraint);
//                    break;
//                default:
//                    DebugPrint(
...
//            BTypedConstraint existingJoint = childGeom.Bones
//                .SelectMany(x => x.GetComponents<BTypedConstraint>())
//                .FirstOrDefault(y => y.otherRigidBody == parentRidgedBody);
//            if (existingJoint)
//            {
//                bone = new GameObject();
...
//                bone.name = jointName;
//                var boneRidgedBody = bone.AddComponent<BRigidBody>();
//                //boneRidgedBody.useGravity = false;
//                joint = bone.AddComponent(jointType) as BTypedConstraint;
//                existingJoint.otherRigidBody = boneRidgedBody;
//                childGeom.Bones.Add(bone);
//            }
//            else
//            {
//                joint = childGeom.Geom.AddComponent(jointType) as BTypedConstraint;
...
//            joint.otherRigidBody = parentRidgedBody;
...
//            if (jointType == typeof(BHingedConstraint))
//                joint = ToConfigurable(joint as BHingedConstraint);

//            joints.Add(new KeyValuePair<string, BTypedConstraint>(jointName, joint));
//            return joints;
//        }
...
//        void ApplyClassToJoint(XElement classElement, BTypedConstraint joint, GeomItem baseGeom, GameObject body, GameObject bone)
//        {
//            BHingedConstraint hingeJoint = joint as BHingedConstraint;
//            BFixedConstraint fixedJoint = joint as BFixedConstraint;
//            B6DOFConstraint configurableJoint = joint as B6DOFConstraint;
...
//        List<MarathonJoint> ParseGears(XElement xdoc, List<KeyValuePair<string, BTypedConstraint>> joints)
//        {
...
//                mujocoJoint.MaximumForce = (mujocoJoint.Joint as B6DOFConstraint).angularXDrive.maximumForce;
...
//        B6DOFConstraint FindTrueBase(BTypedConstraint joint, List<MarathonJoint> mJoints)
//        {
//            B6DOFConstraint configurableJoint = joint as B6DOFConstraint;
//            var rb = configurableJoint.GetComponent<BRigidBody>();
//            //if (rb.useGravity)
//            //    return configurableJoint;
//            B6DOFConstraint parentRb = mJoints
//                    .Select(x => x.Joint)
//                    .First(x => x.otherRigidBody == rb)
//                as B6DOFConstraint;
//            return FindTrueBase(parentRb, mJoints);
//        }

//        Transform FindTrueTarget(BTypedConstraint joint, List<MarathonJoint> mJoints)
//        {
//            var targetRB = joint.otherRigidBody;
//            var rb = joint.GetComponent<BRigidBody>();
//            if (targetRB.useGravity)
//                return targetRB.transform;
//            var target = targetRB.GetComponent<B6DOFConstraint>();
//            if (target == null)
//                return targetRB.transform;
//            return FindTrueTarget(target, mJoints);
//        }

//        List<MarathonJoint> ParseGear(XElement xdoc, List<KeyValuePair<string, BTypedConstraint>> joints)
//        {
//            var mujocoJoints = new List<MarathonJoint>();
...
//            foreach (BTypedConstraint joint in matches)
//            {
//                BHingedConstraint hingeJoint = joint as BHingedConstraint;
//                B6DOFConstraint configurableJoint = joint as B6DOFConstraint;
...
//        public static BTypedConstraint ToConfigurable(BHingedConstraint hingeJoint)
//        {
//            if (hingeJoint.enableMotor)
//            {
//                throw new NotImplementedException();
//            }

//            B6DOFConstraint configurableJoint = hingeJoint.gameObject.AddComponent<B6DOFConstraint>();
