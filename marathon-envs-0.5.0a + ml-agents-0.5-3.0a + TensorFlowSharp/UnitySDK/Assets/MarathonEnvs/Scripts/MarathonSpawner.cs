...
namespace MLAgents
{
    public class MarathonSpawner : MonoBehaviour
    {
        ...
        public int legCount = 4; //csong change
        ...
        //    yield break;
        //}
        public int clientNum = 0; //이 클라이언트가 몇번째 클라이언트인지
        public string SetXml(int legCount)
        {
            string xmlPath = string.Format(Application.streamingAssetsPath + "/N/pybullet_ant_{0}_modified{1}.xml", legCount, clientNum);
            if (!System.IO.File.Exists(xmlPath)) //mod 파일 없음
            {
                xmlPath = string.Format(Application.streamingAssetsPath + "/N/pybullet_ant_{0}.xml", legCount);
            }
            try
            {
                TextAsset asset = new TextAsset(System.IO.File.ReadAllText(xmlPath));
            ...
            catch (Exception ex) //해당 파일 없을 경우 처리
            {
                print("Xml Load Fail : " + ex.Message + "path : " + xmlPath);
            }
        ...
        {
            if (Xml == null)
                Xml = new TextAsset(System.IO.File.ReadAllText(Application.streamingAssetsPath + "/" + "N/pybullet_ant_4.xml"));
