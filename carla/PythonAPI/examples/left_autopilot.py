...
try:
    sys.path.append(glob.glob('../bridgeTest/LeftDrive/PythonAPI/carla/dist/carla-*%d.%d-%s.egg' % (
...
try:
    # server 접속 및 sync 설정
    client = carla.Client("192.168.80.55", 3000)
    ...
    # left drive server 사용이 종료되었으므로 sync 해제
    vehicle.destroy()
