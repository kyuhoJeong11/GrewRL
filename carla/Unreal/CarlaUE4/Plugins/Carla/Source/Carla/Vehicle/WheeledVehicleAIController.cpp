...
void AWheeledVehicleAIController::ConfigureAutopilot(const bool Enable, const bool KeepState)
{
  ...
  if (!KeepState)
  {
	...
	//Vehicle->FLWheelSpeed(0.0f);
	//Vehicle->FRWheelSpeed(0.0f);
	//Vehicle->BLWheelSpeed(0.0f);
	//Vehicle->BRWheelSpeed(0.0f);
    ...
    const bool bOverwriteCurrent)
{
  ...
  for (auto &Location : Locations)
  {
	...
	UE_LOG(LogCarla, Warning, TEXT("SetFixedRoute '%s'"), *Location.ToString());
  }
}
